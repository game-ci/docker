...
const httpie_1 = __nccwpck_require__(386);
...
const action = async () => {
    ...
    if (status === 'started') {
        try {
            ...
            const { statusCode, data } = await (0, httpie_1.post)(startedEndpoint, { headers, body });
    ...
    if (status === 'failed') {
        try {
            ...
            const { statusCode, data } = await (0, httpie_1.post)(failedEndpoint, { headers, body });
    ...
    if (status === 'published') {
        try {
            ...
            const { statusCode, data } = await (0, httpie_1.post)(publishedEndpoint, { headers, body });
...
const { request } = __nccwpck_require__(687);
const { globalAgent } = __nccwpck_require__(685);
const { parse, resolve } = __nccwpck_require__(310);
