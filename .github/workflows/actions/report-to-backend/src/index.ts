...
const action = async () => {
  ...
  if (status === 'started') {
    try {
      ...
      console.error('An error occurred while reporting the start of this build.', error.statusCode, error.message);
  ...
  if (status === 'failed') {
    try {
      ...
      console.error('An error occurred while reporting the build failure.', error.statusCode, error.message);
      console.error('~> data:', error.data);
      core.setFailed(error);
    }
  ...
  if (status === 'published') {
    try {
      ...
      console.error('An error occurred while reporting this publication.', error.statusCode, error.message);
      console.error('~> data:', error.data);
      core.setFailed(error);
    }
